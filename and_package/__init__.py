...
run_time = input("How many iterations do you want:")

j = int(run_time)
print("Make sure that you have set your registration number in login function")
print("")
...
def back_home():
    ...
    home_button.click()
    time.sleep(2)
def course_type(rb,pg_num):
    global ue
    driver.implicitly_wait(5)
    radio_button = driver.find_element_by_xpath(f"//input[@id='registrationOption{rb}']")  #PC = 0 ; PE = 1 ; UC = 2 ; UE = 3
    ...
    ue = rb
    if int(rb) == 3:
        search_button= "//input[@id='searchValue']"
        course_code = input("Enter the course code:")
        driver.find_element_by_xpath("//input[@id='searchValue']").send_keys(course_code)
        driver.find_element_by_xpath("//button[@id='findBtn']").click()
...
def course_reg(name,pg,n):
    ...
    #For UE courses,pg = 1 and n = 4
    print()
    print(f" {name}:-")
    discord_normal_ping(f" {name}:-")
    driver.implicitly_wait(5)
    if ue == 3:
        button_id = "//button[@id='registerbtn_0']"
        time.sleep(1)
        driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
...
def course_view(name,pg,n):
    ...
    if ue == 3:
        driver.find_element_by_xpath("//tbody/tr[1]/td[12]/button[1]").click()
        time.sleep(1)
        driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
        time.sleep(1)
    ...
    try:
        ...
        try:
            ...
            driver.find_element_by_xpath("//button[contains(text(),'Go Back')]").click()
        pass
def faculty_reg(m):     #m =  selected faculty number
    driver.implicitly_wait(10)
